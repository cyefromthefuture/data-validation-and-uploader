...
if 'verified_locations' not in st.session_state:
    st.session_state.verified_locations = set()
...
def check_location_online(destination, expected_country):
    """
    Revised logic: Check strictly if the City/Port exists within the Country.
    """
    try:
        search_query = f"{destination}, {expected_country}"
        location = geolocator.geocode(search_query, language='en')
        
        if location:
            found_address = location.raw.get('address', {})
            found_country = str(found_address.get('country', '')).upper()
            found_code = str(found_address.get('country_code', '')).upper()
            target_country = str(expected_country).upper().strip()
            
            if target_country in found_country or target_country in COUNTRY_ALIASES.get(found_country, []):
                return True, "Match"
            
            if ISO_MAPPING.get(target_country) == found_code or target_country == found_code:
                 return True, "Match"
            
            return True, "Match (Relaxed)"

        return False, f"Could not find '{destination}' inside '{expected_country}'"
    except: 
        return True, "Internet Error"
...
def validate_data(df, use_internet):
    errors = []
    potential_typos = set()
    potential_loc_mismatches = set() # Store Destination strings here
    ...
    if len(df.columns) < 13:
        ...
        return errors, potential_typos, potential_loc_mismatches, checks_passed
        ...
        try:
            ...
            for cntry, dest in unique_pairs:
                ...
                if pd.notna(cntry) and pd.notna(dest):
                    # Skip if this destination is already verified/whitelisted
                    if str(dest).strip().upper() in st.session_state.verified_locations:
                        continue
    ...
    for index, row in df.iterrows():
        ...
        if pd.notna(country) and pd.notna(dest):
             c_str, d_str = str(country).strip().upper(), str(dest).strip().upper()
             
             # 1. Skip if Whitelisted
             if d_str in st.session_state.verified_locations:
                 pass
             # 2. Skip if Code Matches
             elif is_port_code_match(d_str, c_str): 
                 pass
             # 3. Internet Check
             elif use_internet:
                 if (str(country), str(dest)) in location_issues: 
                     # Add to potential list for UI verification instead of hard error
                     potential_loc_mismatches.add(d_str)
                     has_loc_error = True
             # 4. Offline Check
             elif not use_internet:
                 if c_str not in d_str and d_str not in c_str:
                     potential_loc_mismatches.add(d_str)
                     has_loc_error = True
    ...
    checks_passed["Location"] = "⚠️ Review" if has_loc_error else "✅ OK"
    ...
    if st.session_state.run_clicked:
        try:
            ...
            with loader:
                ...
                errors, typos, loc_mismatches, checks_passed = validate_data(df, enable_internet)
            ...
            # 6b. LOCATION UI (SHORTCUT CHECKER)
            unchecked_locs_as_errors = []
            if loc_mismatches:
                st.warning("⚠️ Unknown Location Shortcuts Detected")
                loc_list = sorted(list(loc_mismatches))
                loc_editor_data = pd.DataFrame({
                    "Shortcut (Col P)": loc_list,
                    "Verified": [loc in st.session_state.verified_locations for loc in loc_list]
                })
                edited_loc_df = st.data_editor(
                    loc_editor_data,
                    column_config={
                        "Verified": st.column_config.CheckboxColumn(label="Add to Dictionary?", required=True),
                        "Shortcut (Col P)": st.column_config.TextColumn(label="Location Code")
                    },
                    disabled=["Shortcut (Col P)"],
                    hide_index=True,
                    key="location_editor"
                )
                verified_locs_now = set(edited_loc_df[edited_loc_df["Verified"] == True]["Shortcut (Col P)"])
                st.session_state.verified_locations.update(verified_locs_now)
                
                unchecked_locs = set(edited_loc_df[edited_loc_df["Verified"] == False]["Shortcut (Col P)"])
                if unchecked_locs:
                    for l in unchecked_locs:
                        unchecked_locs_as_errors.append({
                            "Ref (Col B)": "Multiple", 
                            "Column": "P (Dest)", 
                            "Error": f"Unknown Location: '{l}'"
                        })

            all_errors = errors + unchecked_typos_as_errors + unchecked_locs_as_errors
